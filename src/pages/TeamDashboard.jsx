...
export default function TeamDashboard() {
  ...
  const currentReport = useMemo(
    () =>
      team?.currentReportId
        ? reports.find((r) => r.id === team.currentReportId)
        : null,
  ...
  const distance =
    currentReport?.location && team?.location
      ? distanceKm(team.location, currentReport.location)
      : null

  async function handleToggleAvailability() {
    await updateTeam({
      status: team.status === 'offline' ? 'available' : 'offline',
    })
  }
        ...
        >
          <div>
            <p className="text-xs font-medium text-ink-400">
              {teamType ? t(teamType.labelKey) : team?.type} ·{' '}
              {t('team.dashboard.statusLabel')}
            </p>
            <p
              className={`mt-0.5 text-sm font-bold ${STATUS_TEXT[teamStatus.theme]}`}
            >
          ...
          {team?.status !== 'busy' && (
            <Button
              size="sm"
              variant={team?.status === 'offline' ? 'primary' : 'secondary'}
              onClick={handleToggleAvailability}
            >
              {team?.status === 'offline'
                ? t('team.dashboard.goAvailable')
                : t('team.dashboard.goOffline')}
            </Button>
          )}
        </motion.div>
          ...
          {!currentReport ? (
            ...
            >
              ...
              <div>
                ...
                {category && (
                  <p className="text-xs text-ink-400">{t(category.labelKey)}</p>
                )}
              </div>
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                ...
                >
                  {completing
                    ? t('team.dashboard.completing')
                    : t('team.dashboard.complete')}
                </Button>
              </div>
            </motion.div>
          )}
