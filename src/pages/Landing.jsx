...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-ink-900 text-ink-900 dark:text-ink-50">
      ...
      <main className="bg-gradient-to-b from-brand-50/70 via-white to-white">
        <section className="mx-auto max-w-4xl px-4 pb-16 pt-16 text-center sm:px-6 sm:pt-24">
          ...
          <motion.h1
            ...
            className="mt-5 font-display text-3xl font-bold leading-tight text-ink-900 dark:text-ink-50 sm:text-5xl"
          >
          ...
          <motion.p
            ...
            className="mx-auto mt-4 max-w-xl text-base text-ink-500 dark:text-ink-400 sm:text-lg"
          >
          ...
          <motion.p
            ...
            className="mt-4 text-xs font-medium text-ink-400 dark:text-ink-500"
          >
          ...
          <motion.div
            ...
            className="mx-auto mt-14 grid max-w-lg grid-cols-3 gap-4 border-t border-ink-200 dark:border-ink-700 pt-8"
          >
            {stats.map((stat) => (
              <div key={stat.key}>
                ...
                <p className="mt-1 text-xs text-ink-500 dark:text-ink-400 sm:text-sm">{t(stat.key)}</p>
              </div>
            ))}
          </motion.div>
        </section>

        <section id="how-it-works" className="border-t border-ink-100 dark:border-ink-800 bg-white dark:bg-ink-900 py-16 sm:py-20">
          <div className="mx-auto max-w-5xl px-4 text-center sm:px-6">
            <span className="eyebrow-pill bg-accent-500/10 text-accent-600">{t('landing.how.eyebrow')}</span>
            <h2 className="mt-4 font-display text-2xl font-bold text-ink-900 dark:text-ink-50 sm:text-3xl">
              {t('landing.how.title')}
            </h2>
            ...
            <div className="mt-10 grid gap-6 sm:grid-cols-3">
              {HOW_STEPS.map((step, i) => (
                <motion.div
                  ...
                  className="rounded-2xl border border-ink-200 dark:border-ink-700 bg-white dark:bg-ink-900 p-8 text-center"
                >
                  <span className={`mx-auto grid h-14 w-14 place-items-center rounded-xl ${step.badge}`}>
                    <step.icon className="h-6.5 w-6.5" />
                  </span>
                  <h3 className="mt-4 font-bold text-ink-900 dark:text-ink-50">{t(step.titleKey)}</h3>
                  <p className="mt-1.5 text-sm text-ink-500 dark:text-ink-400">{t(step.bodyKey)}</p>
                </motion.div>
              ))}
            </div>
          </div>
        ...
        <section id="categories" className="border-t border-ink-100 dark:border-ink-800 bg-ink-50 dark:bg-ink-950 py-16 sm:py-20">
          <div className="mx-auto max-w-5xl px-4 text-center sm:px-6">
            <span className="eyebrow-pill bg-brand-600/10 text-brand-700">{t('landing.categories.eyebrow')}</span>
            <h2 className="mt-4 font-display text-2xl font-bold text-ink-900 dark:text-ink-50 sm:text-3xl">
              {t('landing.categories.title')}
            </h2>
            <p className="mx-auto mt-2 max-w-xl text-sm text-ink-500 dark:text-ink-400 sm:text-base">
              ...
              {CATEGORIES.map((category, i) => {
                return (
                  <motion.button
                    ...
                    whileHover={{ y: -4 }}
                    className="flex h-full flex-col justify-between rounded-2xl border border-ink-200 dark:border-ink-700 bg-white dark:bg-ink-900 p-8 text-left transition-shadow duration-300 hover:shadow-card-hover"
                  >
                    <div>
                    ...
                    <div className={`mt-6 flex items-center gap-1.5 text-sm font-semibold ${CATEGORY_CTA[category.theme]}`}>
        ...
        <section className="px-4 py-16 sm:px-6 sm:py-20">
          ...
          >
            <h2 className="font-display text-2xl font-bold sm:text-3xl">{t('landing.cta.banner.title')}</h2>
      ...
      <footer className="border-t border-ink-100 dark:border-ink-800 bg-white dark:bg-ink-900 py-8">
        <div className="mx-auto flex max-w-6xl flex-col items-center gap-3 px-4 text-center sm:px-6">
          ...
          <p className="text-xs text-ink-400 dark:text-ink-500">{t('landing.footer.builtFor')}</p>
        </div>
      </footer>
    </div>
  )
