...
import { IconChevronLeft, IconCheck, IconAlertCircle, IconLogOut } from '../components/Icons'
...
export default function AdminAddTeam() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setError(err?.message === 'ID_TAKEN' ? t('admin.teams.error.idTaken') : t('admin.teams.error'))
    } finally {
      setBusy(false)
    }
  }
  ...
  if (!isFirebaseConfigured) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-ink-50 dark:bg-ink-950 px-4 py-10">
        <div className="w-full max-w-md rounded-2xl border border-ink-200 dark:border-ink-700 bg-white dark:bg-ink-900 p-8 text-center shadow-card-hover">
          <p className="text-sm text-ink-500 dark:text-ink-400">{t('admin.teams.unavailable')}</p>
          <Button className="mt-5" variant="secondary" onClick={() => navigate('/admin')}>
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-ink-50 dark:bg-ink-950 px-4 py-10">
        ...
        >
          ...
          <h1 className="mt-4 text-lg font-bold text-ink-900 dark:text-ink-50">{t('admin.teams.created.title')}</h1>
          <p className="mt-2 text-sm text-ink-600 dark:text-ink-300">{t('admin.teams.created.customNote')}</p>

          <div className="mt-4 rounded-xl bg-white dark:bg-ink-900 px-4 py-3 text-left">
            <p className="text-[11px] font-medium uppercase tracking-wide text-ink-400 dark:text-ink-500">
              {t('admin.teams.created.idLabel')}
            </p>
            <p className="font-mono text-sm font-semibold text-ink-900 dark:text-ink-50">{created.id}</p>
          </div>
  ...
  return (
    <div className="min-h-screen bg-ink-50 dark:bg-ink-950">
      ...
      <main className="mx-auto max-w-2xl px-4 py-8 sm:px-6 sm:py-12">
        <h1 className="font-display text-2xl font-bold text-ink-900 dark:text-ink-50">{t('admin.teams.addNew')}</h1>
        <p className="mt-1.5 text-ink-500 dark:text-ink-400">{t('admin.teams.addNewSubtitle')}</p>

        <form
          onSubmit={handleSubmit}
          className="mt-6 space-y-4 rounded-2xl border border-ink-200 dark:border-ink-700 bg-white dark:bg-ink-900 p-6 shadow-card sm:p-8"
        >
          <label className="block">
            <span className="mb-1.5 block text-sm font-medium text-ink-700 dark:text-ink-200">{t('admin.teams.nameLabel')}</span>
            <input
              value={name}
          ...
          <label className="block">
            <span className="mb-1.5 block text-sm font-medium text-ink-700 dark:text-ink-200">{t('admin.teams.typeLabel')}</span>
          ...
          <label className="block">
            <span className="mb-1.5 block text-sm font-medium text-ink-700 dark:text-ink-200">{t('admin.teams.areaLabel')}</span>
            ...
            <p className="mt-1.5 text-xs text-ink-400 dark:text-ink-500">{t('admin.teams.areaHint')}</p>
          </label>

          <div className="grid gap-4 sm:grid-cols-2">
            <label className="block">
              <span className="mb-1.5 block text-sm font-medium text-ink-700 dark:text-ink-200">{t('admin.teams.idLabel')}</span>
              <input
                value={id}
          ...
          <p className="text-xs text-ink-400 dark:text-ink-500">{t('admin.teams.idPasscodeHint')}</p>
